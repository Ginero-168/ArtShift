...
/**
 * Keyword → LLM vibe / design-direction JSON + exactly 9 image prompts.
 * Does not call image.generate — generation is a separate Moodboard route.
 */
...
function looksLikeExpandPack(value: unknown): boolean {
  ...
  return (
    typeof value.keyword === "string" || isRecord(value.roles) || Array.isArray(value.imagePrompts)
  );
}
