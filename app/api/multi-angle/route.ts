...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (error instanceof RequestBodyTooLargeError) {
      return invalidRequest("Multi-Angle request is too large.", 413);
    }
    return invalidRequest("Invalid JSON body.");
  }

  if (!isRecord(body) || body.task !== "image.multiAngle") {
    return invalidRequest("This endpoint only supports Qwen Edit Multi-Angle.");
  }
  ...
  try {
    ...
    const execution = await ai.execute("image.multiAngle", request.input, {
      profile: "quality",
      provider: "replicate",
