...
import { fetchPinterestBoards, PinterestApiError } from "@/lib/server/pinterest/client";
import {
  applyCookies,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return applyCookies(bag, jsonNoStore({ boards, connected: true }));
  } catch (error) {
    ...
    return applyCookies(
      bag,
      jsonNoStore({ error: "Could not load boards from Pinterest." }, { status: 502 }),
    );
  }
}
