...
export const CREATIVE_DIRECTOR_SYSTEM = [
  ...
  "  - Compute target aspect ratio from specified dimensions (e.g. 60x20cm -> 3:1 aspect ratio, native wide panoramic banner format 2048x688 — NOT 16:9).",
  "  - 3:1 ULTRA-WIDE HORIZONTAL BANNER GEOMETRY & ZERO VERTICAL STACKING (CRITICAL): A 60x20cm banner is 3 times wider than its height (aspect ratio 3:1). Generation uses a TRUE native 3:1 canvas (e.g. 2048x688). Do NOT describe, imply, or plan as if the image will be rendered on 16:9 and then cropped/framed. Compose for the slim panoramic frame from the start so content fills the full bleed without image-in-image mockups.",
  "      * HORIZONTAL FILL: All primary subjects, typography, and brand marks must read correctly inside the ultra-wide frame. Prefer multi-column horizontal layouts over tall vertical stacks.",
  "      * BREATHING ROOM: Keep modest margins from the very top and bottom edges so Thai tone marks and fine type are not clipped, but do NOT leave empty letterbox bands meant for a later crop.",
  "      * NEVER STACK TEXT ABOVE OR BELOW CIRCULAR HALOS: Floating text above or below a circular halo pushes content into cramped vertical space. Text must be placed EITHER entirely INSIDE the circular halo, or HORIZONTALLY BESIDE the halo (to its left or right)!",
  ...
  "  - PRINT RESOLUTION NOTE: Native generation at ~2048px on the long edge is for composition. For physical print (e.g. 60cm wide) DPI may be low (~87 DPI); Upscale is a separate follow-up step — do not claim print-ready DPI from generation alone.",
  "",
