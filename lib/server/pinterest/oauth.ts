...
import { getPublicAppUrl, getRequestHostname } from "@/lib/server/auth/google";
...
export function pinterestSetupMessage(): string {
  ...
  return `Set PINTEREST_CLIENT_ID, PINTEREST_CLIENT_SECRET, and ARTSHIFT_PUBLIC_URL. Register the redirect URI ${publicUrl}/api/pinterest/oauth/callback on the Pinterest app (display name ${appName}). Production example: https://artshift.io/api/pinterest/oauth/callback.`;
}
...
export function safePinterestReturnTo(value: string | null | undefined): string {
  ...
  const path = value.split("?")[0]?.split("#")[0] ?? "/projects";
  if (path.length > 240) return "/projects";
  return path;
}

/**
 * `__Host-` OAuth state cookies are bound to the host that set them. Bounce to the
 * canonical public URL before setting a cookie when the request host differs.
 */
export function getCanonicalPinterestStartRedirect(
  ...
  return `${publicUrl}/api/pinterest/oauth/start`;
}
...
export async function exchangePinterestCode(
  ...
  if (!code || code.length > 2_048) throw new Error("Invalid Pinterest authorization code.");
  ...
  return requestToken(config, body, signal);
}
...
export async function refreshPinterestToken(
  ...
  return requestToken(config, body, signal, refreshToken);
}
...
async function requestToken(
  ...
  try {
    response = await fetch(PINTEREST_TOKEN_ENDPOINT, {
      method: "POST",
      headers: {
    ...
    throw new Error("Pinterest authorization is temporarily unavailable.");
  }
  ...
  return tokensFromPayload(payload, fallbackRefresh);
}
...
function decryptState(value: string): PinterestState | null {
  ...
  try {
    ...
    return { state: parsed.state, returnTo: parsed.returnTo, exp: parsed.exp };
  } catch {
    return null;
  }
}
