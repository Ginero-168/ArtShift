...
export function setPinterestSessionCookie(
  ...
  const maxAge = Math.max(
    60,
    Math.min(
      PINTEREST_SESSION_TTL_SECONDS,
      Math.floor((tokens.expiresAt - Date.now()) / 1_000) + PINTEREST_SESSION_TTL_SECONDS,
    ),
  );
...
export async function ensurePinterestAccess(
  ...
  try {
    ...
    const claims: PinterestSessionClaims = { ...next, username: session.username };
