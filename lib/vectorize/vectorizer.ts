...
/**
 * Vectorizes a raster image into highly detailed, layered ArtShift VectorPathElements.
 */
export async function vectorizeImage(
  ...
  const numColors = mode === "monochrome" ? 2 : (options?.colors ?? defaults.colors);
  const detailLevel = options?.detailLevel ?? defaults.detailLevel;
  const smoothing = options?.smoothing ?? defaults.smoothing;
  const cornerSharpness = options?.cornerSharpness ?? defaults.cornerSharpness;
  const minArea = options?.minArea ?? defaults.minArea;
  const blackThreshold = options?.blackThreshold ?? defaults.blackThreshold;

  // Resolution scaling based on Detail Level (1..5)
  // Level 1 = 450px (Fast)
  // Level 2 = 650px (Standard)
  // Level 3 = 900px (High Quality)
  // Level 4 = 1200px (Very High Detail)
  // Level 5 = 1600px (Ultra Photorealistic Fidelity)
  const maxDimensions = [450, 650, 900, 1200, 1600];
  const maxDimension = maxDimensions[detailLevel - 1] ?? 1000;
  ...
  const { palette, map } = quantizeColors(imgData.data, w, h, numColors, detailLevel);
  ...
  for (let c = 0; c < palette.length; c++) {
    ...
    // Skip white background in monochrome mode
    if (mode === "monochrome" && lum > blackThreshold) {
      continue;
    }
    ...
    const rawContours = traceContours(binaryMask, w, h, minArea);
    ...
    for (const contour of rawContours) {
      ...
      const nodes = fitCornerPreservingBezierNodes(simplified, w, h, smoothing, cornerSharpness);
  ...
  const svgString = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${targetBounds.width} ${targetBounds.height}">\n${svgPaths}</svg>`;
