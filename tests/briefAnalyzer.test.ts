...
describe("Dynamic Image Layout Analyzer (briefAnalyzer)", () => {
  it("computes dynamic aspect ratios and layout partitions instead of static mocks", async () => {
    // 16:9 ratio test
    const wideData = await analyzeImageDynamic("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAABCAYAAAD0In+KAAAAEUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==", 1600, 900);
  ...
  it("produces different divider positions and aspect ratios for portrait vs landscape", async () => {
    const portraitData = await analyzeImageDynamic("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAACCAYAAACF9TxKAAAAEUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==", 600, 1000);
    const landscapeData = await analyzeImageDynamic("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAABCAYAAAD0In+KAAAAEUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==", 1200, 600);
