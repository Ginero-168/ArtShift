...
describe("Moodboard expand API", () => {
  ...
  it("requires auth, consent, and BYOK", async () => {
    ...
    const unauth = await expandPost(request({ keyword: "ice", cloudConsent: true }));
    expect(unauth.status).toBe(401);

    accountMock.value = { id: "account-test" };
    const noConsent = await expandPost(request({ keyword: "ice" }));
    expect(noConsent.status).toBe(403);

    replicateTokenMock.value = null;
    const noKey = await expandPost(request({ keyword: "ice", cloudConsent: true }));
  ...
  it("returns a parsed 9-prompt pack from assistant.chat", async () => {
    ...
    const response = await expandPost(request({ keyword: "Bangkok", cloudConsent: true }));
...
describe("Moodboard generate API", () => {
  ...
  it("generates via flux-schnell alias only", async () => {
    runtimeMock.execute.mockResolvedValue({
      output: {
    ...
    expect(body.defaultModel).toBe("black-forest-labs/flux-schnell");
    expect(body.estimatedUsd).toBe(0.003);
    expect(runtimeMock.execute).toHaveBeenCalledWith(
      "image.generate",
      expect.objectContaining({ modelAlias: "flux-schnell", enhance: false }),
      expect.objectContaining({
